...
(function() {
  this.mapIconMsgType5 = function(reading) {
    if (reading.dtcCodes) {
      return "https://chart.googleapis.com/chart?chst=d_simple_text_icon_above&chld=Engine|9|000|caution|12|C21B16|FFF";
    } else {
      return "https://mts0.google.com/vt/icon/name=icons/spotlight/directions_decision_point_6px.png&scale=1";
    }
  };

  this.mapIcon = function(eventCode, speed) {
    var icon, iconUrl, icons;
    eventCode = String(eventCode);
    iconUrl = (function() {
      switch (eventCode) {
        case '20':
          return _.template("https://mts.googleapis.com/vt/icon/name=icons/spotlight/spotlight-waypoint-a.png&text=<%= text %>&psize=13&font=fonts/Roboto-Regular.ttf&color=<%= color %>&ax=44&ay=51&scale=1");
        case '30':
          return _.template("https://mts.googleapis.com/vt/icon/name=icons/spotlight/spotlight-waypoint-b.png&text=<%= text %>&psize=13&font=fonts/Roboto-Regular.ttf&color=<%= color %>&ax=41&ay=51&scale=1");
        case '50':
        case '51':
          return _.template("https://chart.googleapis.com/chart?chst=d_simple_text_icon_above&chld=<%= text %>|10|000|glyphish_location|16|<%= color %>|fff");
        case '21':
        case '22':
          return _.template("https://mts0.google.com/vt/icon/name=icons/spotlight/directions_decision_point_6px.png&scale=1");
        default:
          return _.template("https://chart.googleapis.com/chart?chst=d_map_spin&chld=0.3|0|<%= color %>|9|b|||<%= text %>");
      }
    })();
    icons = {
      '20': {
    ...
    if (icon = icons[eventCode]) {
      return iconUrl({
        color: icon.color,
        text: icon.text || ("" + (Number(speed).toFixed()) + " mph")
      });
    }
  };
...
(function() {
  ...
  Zin.Form = (function(fn) {
    var _addEmailTip, _emailInputId, _origEmailEditor, _origEmailInput, _origEmailValue;
    ...
    return {
      ...
      handleKeyUp: function(event) {
        ...
        return this.exeKeyUp(ele, keycode);
      },
      exeKeyUp: function(ele, keycode) {
        var emailValue;
        emailValue = $(ele).val().replace(/^\s+|\s+$/g, "");
        if (emailValue !== "" && (keycode === 32 || keycode === 188)) {
          this.createNewEmail(emailValue, ele);
          $(ele).val("");
        }
        return this.calculateEmails();
      },
      handleBlur: function(event) {
        ...
        ele = event.target;
        emailValue = $(ele).val().replace(/^\s+|\s+$/g, "");
        if (emailValue && ele === this.getLastEmailInput()) {
          this.exeKeyUp(ele, 188);
        }
        if (!emailValue && ele !== this.getLastEmailInput()) {
          return this.deleteEmail(ele);
        }
      },
      createEmailEditor: function() {
        var emailValues, i;
        window._emailEditor = $("<div></div>").attr({
          id: "emailEditor"
        });
        if (_origEmailValue) {
          ...
          while (i < emailValues.length) {
            if (emailValues[i]) {
              this.createNewEmail(emailValues[i]);
            }
            i++;
          }
        }
        this.createNewEmail();
        return $(_emailEditor).insertAfter(_origEmailEditor);
      },
      createNewEmail: function(emailValue, ele) {
        var newEmailDiv, newEmailInput;
        newEmailDiv = $("<div></div>").addClass("input-group").attr({
          id: "emailInput" + _emailInputId
        });
        newEmailInput = $("<input>").attr({
          ...
          title: _addEmailTip
        }).addClass("form-control").keyup(function(event) {
          return Zin.Form.handleKeyUp(event);
        }).blur(function(event) {
          return Zin.Form.handleBlur(event);
        });
        newEmailDiv.append(newEmailInput);
        if (emailValue) {
          newEmailInput.val(emailValue);
          this.createDelete(newEmailDiv, newEmailInput, _emailInputId);
        } else {
          newEmailInput.attr({
      ...
      createDelete: function(newEmailDiv, newEmailInput, _emailInputId) {
        var deleteBttn, deleteBttnHtml, deleteBttnWrapper;
        if (!_emailInputId) {
          _emailInputId = $(newEmailInput).attr("id").charAt(3);
        }
        deleteBttnWrapper = $("<span></span>").addClass('input-group-btn');
        deleteBttn = $("<btn></btn>").attr({
          id: "delete" + _emailInputId
        }).addClass("btn btn-default").click(function() {
          return Zin.Form.deleteEmail(this);
        });
        ...
        return $(newEmailDiv).append(deleteBttnWrapper);
      },
      divToDelete: false,
      deleteEmail: function(deleteBttn) {
        this.divToDelete = $(deleteBttn).parent();
        $(this.divToDelete).addClass("exeDelete");
        return window.setTimeout("Zin.Form.exeDeleteEmail()", 500);
      },
      exeDeleteEmail: function() {
        $(Zin.Form.divToDelete).remove();
        return Zin.Form.calculateEmails();
      },
      getLastEmailInput: function() {
        var i;
        i = _emailInputs.length - 1;
        return _emailInputs[i];
      },
      calculateEmails: function() {
        var emailValue, emailValuesStr, i;
        ...
        while (i < _emailInputs.length) {
          ...
          if (emailValue && emailValue !== _addEmailTip) {
            emailValuesStr += emailValue + ",";
          }
          i++;
        }
        return $(_origEmailInput).val(emailValuesStr.replace(/^\s+|\s+$/g, ""));
      }
    };
  })(Zin.Form || {});

  $(document).ready(function() {
    return Zin.Form.initEmailEditor();
  });
